...
class ShoulderPressDetector(BaseExercise):
   ...
   def process(self, landmarks) -> dict:
      ...
      elbow_angle = self.calculate_angle(
         ...
         self.get_pont(landmarks, elbow_idx),
         self.get_point(landmarks, wrist_idx)
      )

      key_landmarks_visible = (landmarks[shoulder_idx].visiblity >= self.MIN_VISIBILITY and
      landmarks[wrist_idx] >= self.MIN_VISIBILITY and landmarks[elbow_idx].visibility >= self.MIN_VISIBILITY )
      ...
      if elbow_angle >= self.UP_THRESHOLD:
         extension_status = "GOOD EXTENSION"
